  ...
  return(
    <div style={{marginTop:'48px'}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'20px'}}>
        ...
        <button onClick={()=>navigate('/goal-races')} style={{display:'flex',alignItems:'center',gap:'6px',padding:'7px 16px',background:'none',border:`1.5px solid ${t.border}`,borderRadius:'8px',fontFamily:"'Barlow Condensed',sans-serif",fontSize:'11px',fontWeight:600,letterSpacing:'1.5px',color:t.textMuted,cursor:'pointer',textTransform:'uppercase',transition:'all 0.15s'}} onMouseEnter={e=>{e.currentTarget.style.borderColor=t.text;e.currentTarget.style.color=t.text}} onMouseLeave={e=>{e.currentTarget.style.borderColor=t.border;e.currentTarget.style.color=t.textMuted}}>Edit Goal</button>
      </div>
      <div style={{background:t.surface,border:`1.5px solid ${t.border}`,borderRadius:'14px',overflow:'hidden',marginBottom:'16px'}}>
        <div style={{padding:'20px 24px',display:'flex',alignItems:'center',gap:'20px'}}>
          ...
          <button onClick={()=>navigate('/discover')} style={{padding:'7px 14px',border:`1.5px solid ${t.text}`,borderRadius:'8px',background:'rgba(27,42,74,0.06)',fontFamily:"'Barlow Condensed',sans-serif",fontSize:'11px',fontWeight:600,letterSpacing:'1px',color:t.text,cursor:'pointer',textTransform:'uppercase',transition:'all 0.15s',whiteSpace:'nowrap',flexShrink:0}} onMouseEnter={e=>{e.currentTarget.style.background='#1B2A4A';e.currentTarget.style.color='#fff'}} onMouseLeave={e=>{e.currentTarget.style.background='rgba(27,42,74,0.06)';e.currentTarget.style.color=t.text}}>View Races →</button>
        </div>
      </div>
    </div>
  )
...
export default function Passport(){
  ...
  const[profile,setProfile]=useState(null),[showDropdown,setShowDropdown]=useState(false),[filter,setFilter]=useState('ALL')
  ...
  useEffect(()=>{
    ...
    loadProfile()
    const style=document.createElement('style');style.id='rp-passport-styles'
      ...
      <div style={{padding:'28px 40px 80px'}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'24px',flexWrap:'wrap',gap:'12px'}}>
          <div style={{display:'flex',gap:'8px',flexWrap:'wrap'}}>
            {FILTERS.map(f=><button key={f} onClick={()=>setFilter(f)} style={{padding:'6px 16px',borderRadius:'20px',border:`1.5px solid ${filter===f?'#1B2A4A':t.border}`,background:filter===f?'#1B2A4A':t.surface,fontFamily:"'Barlow Condensed',sans-serif",fontSize:'11px',fontWeight:600,letterSpacing:'1.5px',textTransform:'uppercase',cursor:'pointer',transition:'all 0.15s',color:filter===f?'#fff':t.textMuted}}>{f}</button>)}
          </div>
          <button onClick={()=>navigate('/race-import')} style={{display:'flex',alignItems:'center',gap:'8px',padding:'8px 20px',background:'#1B2A4A',border:'none',borderRadius:'8px',fontFamily:"'Barlow Condensed',sans-serif",fontSize:'12px',fontWeight:600,letterSpacing:'1.5px',color:'#fff',cursor:'pointer',textTransform:'uppercase',transition:'background 0.15s'}} onMouseEnter={e=>e.currentTarget.style.background='#C9A84C'} onMouseLeave={e=>e.currentTarget.style.background='#1B2A4A'}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:'20px'}}>
          {filteredRaces.map((race,i)=><PassportCard key={race.id} race={race} index={i} t={t} onClick={()=>navigate(`/race/${race.id}`)}/>)}
        </div>
