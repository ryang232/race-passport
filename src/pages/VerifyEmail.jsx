...
export default function VerifyEmail() {
  ...
  const { email = '', name = '', firstName: passedFirst = '', isNewUser = true, nextStep = 'create-account' } = location.state || {}
  ...
  const firstName = passedFirst || (name ? name.split(' ')[0] : '')
  ...
  useEffect(() => {
    ...
    style.textContent = `
      ...
      .step-dot { width:26px; height:26px; borderRadius:50%; display:flex; align-items:center; justify-content:center; flex-shrink:0; font-size:12px; transition:all 0.4s ease; }
      .step-dot.done { background:#C9A84C; color:#fff; border-radius:50%; }
      .step-dot.done-anim { background:#C9A84C; color:#fff; border-radius:50%; animation:checkPop 0.5s ease forwards; }
      .step-dot.pending { background:#f0f2f5; color:#b0b8c4; border:1.5px solid #e2e6ed; border-radius:50%; }
    `
  ...
  const handleVerify = async () => {
    if (!code || code.length < 6) { setError('Please enter the full 6-digit code from your email'); return }
    setError(null)
    setLoading(true)

    // Try signup OTP type first, fall back to email type
    ...
    if (result.error) {
      setError('That code didn\'t work — please check and try again, or tap Resend.')
      return
    }

    setVerified(true)
    setTimeout(() => {
      navigate(`/${nextStep}`, { state: { email, firstName } })
    }, 1800)
  }
      ...
      <div style={{ position:'relative', zIndex:10, background:'#fff', borderRadius:'4px', padding:'40px 36px 32px', width:'100%', maxWidth:'380px', margin:'20px', boxShadow:'0 2px 40px rgba(27,42,74,0.10),0 0 0 1px rgba(27,42,74,0.07)' }}>
        ...
        <div style={{ textAlign:'center', marginBottom:'24px' }}>
          ...
          <div style={{ width:52, height:52, borderRadius:'50%', background:'rgba(201,168,76,0.1)', border:'1.5px solid rgba(201,168,76,0.3)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 16px', animation: verified ? 'pulse 1s ease infinite' : 'none' }}>
            {verified
              ? <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M5 13l4 4L19 7" stroke="#C9A84C" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
          ...
          {!verified && (
            <>
              ...
              <p style={{ fontSize:'13px', color:'#1B2A4A', fontWeight:600, margin:'0 0 8px' }}>{email}</p>
              <p style={{ fontSize:'12px', color:'#9aa5b4', margin:0, fontWeight:300 }}>Enter it below to continue creating your account.</p>
            </>
          )}

          {verified && (
            <p style={{ fontSize:'13px', color:'#9aa5b4', margin:0, fontWeight:300, lineHeight:1.6 }}>
        ...
        {!verified && (
          <>
            <div style={{ marginBottom:'16px' }}>
              ...
              <input className="rp-code-input" type="text" inputMode="numeric" maxLength={8}
                value={code} onChange={e => { setCode(e.target.value.replace(/\D/g,'')); setError(null) }}
                placeholder="· · · · · ·"
                onKeyDown={e => e.key === 'Enter' && handleVerify()} />
            </div>

            <button className="rp-primary" onClick={handleVerify}
              disabled={loading || code.length < 6}
              style={{ marginBottom:'14px', display:'flex', alignItems:'center', justifyContent:'center', gap:'8px' }}>
              {loading
                ? <><div style={{ width:14, height:14, border:'2px solid rgba(255,255,255,0.3)', borderTopColor:'#fff', borderRadius:'50%', animation:'spin 0.7s linear infinite' }}/> Verifying...</>
            ...
            <p style={{ textAlign:'center', color:'#9aa5b4', fontSize:'12px', margin:0, fontWeight:300 }}>
              ...
              <span onClick={handleResend}
                style={{ color:'#C9A84C', cursor: resending ? 'not-allowed' : 'pointer', fontWeight:600 }}>
