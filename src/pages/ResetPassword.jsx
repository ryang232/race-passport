...
export default function ResetPassword() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      document.getElementById('rp-rp-styles')?.remove()
    }
  }, [])
      ...
      <div style={{ position: 'relative', zIndex: 10, background: '#fff', borderRadius: '4px', padding: '40px 36px 32px', width: '100%', maxWidth: '380px', margin: '20px', boxShadow: '0 2px 40px rgba(27,42,74,0.10), 0 0 0 1px rgba(27,42,74,0.07)' }}>
        ...
        {!done ? (
          <>
            ...
            {!sessionReady && (
              <div style={{ background: 'rgba(27,42,74,0.06)', border: '1px solid rgba(27,42,74,0.2)', borderRadius: '6px', padding: '10px 14px', fontSize: '13px', marginBottom: '14px', color: '#1B2A4A', lineHeight: 1.6 }}>
                Waiting for reset link to activate... If this persists, request a new reset link.
              </div>
            )}
            ...
            <button className="rp-primary" onClick={handleReset} disabled={loading || !sessionReady}>
          ...
          <>
            ...
            <button className="rp-primary" onClick={() => navigate('/login')}>
              Go to Sign In →
            </button>
          </>
        )}
