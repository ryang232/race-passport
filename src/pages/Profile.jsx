...
function DobPicker({ value, onChange, t }) {
  ...
  const sel = { padding:'9px 12px', borderRadius:'6px', border:`1.5px solid ${t.border}`, background:t.inputBg, color:t.text, fontSize:'13px', fontFamily:"'Barlow',sans-serif", outline:'none', appearance:'none', cursor:'pointer', width:'100%' }
  ...
  return (
    <div style={{ display:'grid', gridTemplateColumns:'1.8fr 1fr 1.4fr', gap:'8px' }}>
      <select value={month} style={sel} onChange={e => { setMonth(e.target.value); fire(e.target.value,day,year) }}><option value="">Month</option>{MONTHS.map((m,i) => <option key={m} value={i+1}>{m}</option>)}</select>
      <select value={day} style={sel} onChange={e => { setDay(e.target.value); fire(month,e.target.value,year) }}><option value="">Day</option>{days.map(d => <option key={d} value={d}>{d}</option>)}</select>
      <select value={year} style={sel} onChange={e => { setYear(e.target.value); fire(month,day,e.target.value) }}><option value="">Year</option>{years.map(y => <option key={y} value={y}>{y}</option>)}</select>
    </div>
  )
}
      ...
      {['Yes','No'].map(opt => {
        ...
        return (
          <button key={opt} onClick={() => onChange(opt==='Yes')}
            style={{ flex:1, padding:'10px', border:'1.5px solid', borderRadius:'6px', fontFamily:"'Barlow Condensed',sans-serif", fontSize:'13px', fontWeight:600, letterSpacing:'1px', cursor:'pointer', transition:'all 0.15s', background:active?'#1B2A4A':t.inputBg, borderColor:active?'#1B2A4A':t.border, color:active?'#fff':t.textMuted }}>
            {opt}
          </button>
        )
      })}
...
export default function Profile() {
  ...
  const Section = ({ title, children, danger }) => (
    <div style={{ background:t.surface, borderRadius:'16px', border:danger?`1.5px solid rgba(197,48,48,0.2)`:`1px solid ${t.border}`, marginBottom:'16px', overflow:'hidden' }}>
      <div style={{ padding: isMobile ? '14px 16px 10px' : '18px 24px 14px', borderBottom:`1px solid ${danger?'rgba(197,48,48,0.1)':t.borderLight}` }}>
        <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:'20px', color:danger?'#c53030':t.text, letterSpacing:'1px' }}>{title}</div>
      </div>
      <div style={{ padding: isMobile ? '14px 16px' : '20px 24px' }}>{children}</div>
    </div>
  )
  ...
  const SaveButton = ({ section, updates }) => (
    <div style={{ display:'flex', justifyContent:'flex-end', marginTop:'14px' }}>
      <button disabled={savingSection===section} onClick={() => saveSection(section, updates)}
    ...
    <div style={{ minHeight:'100vh', background:t.bg, fontFamily:"'Barlow',sans-serif", transition:'background 0.25s', overflowX:'hidden' }}>
      ...
      <div style={{ position:'sticky', top:0, zIndex:50, background:t.navBg, backdropFilter:'blur(10px)', borderBottom:`1px solid ${t.navBorder}`, boxShadow:t.navShadow }}>
        {isMobile ? (
          ...
          <div style={{ display:'flex', alignItems:'stretch', justifyContent:'space-between', padding:'0 40px' }}>
            ...
            <div style={{ display:'flex', alignItems:'center' }}>
              <div ref={dropdownRef} style={{ position:'relative' }}>
                <div onClick={() => setShowDropdown(!showDropdown)} style={{ width:40, height:40, borderRadius:'50%', background:'#1B2A4A', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', border:`2px solid ${t.border}` }} onMouseEnter={e => e.currentTarget.style.borderColor='#C9A84C'} onMouseLeave={e => e.currentTarget.style.borderColor=t.border}>
                ...
                {showDropdown && (
                  <div style={{ position:'absolute', right:0, top:'calc(100% + 8px)', background:t.surface, border:`1px solid ${t.border}`, borderRadius:'10px', boxShadow:t.cardShadowHover, minWidth:'200px', overflow:'hidden', zIndex:100 }}>
                    ...
                    <button style={{ display:'block', width:'100%', padding:'10px 18px', background:'none', border:'none', textAlign:'left', fontFamily:"'Barlow Condensed',sans-serif", fontSize:'13px', fontWeight:600, color:t.text, cursor:'pointer' }} onMouseEnter={e => e.currentTarget.style.background=t.surfaceAlt} onMouseLeave={e => e.currentTarget.style.background='transparent'} onClick={() => { navigate('/passport'); setShowDropdown(false) }}>My Passport</button>
      ...
      {/* PAGE HEADER */}
      <div style={{ background:'#1B2A4A', padding: isMobile ? '20px 16px' : '32px 40px', position:'relative', overflow:'hidden' }}>
        {!isMobile && <div style={{ position:'absolute', right:0, top:'-10px', fontFamily:"'Bebas Neue',sans-serif", fontSize:'160px', color:'transparent', WebkitTextStroke:'1px rgba(255,255,255,0.04)', lineHeight:1, userSelect:'none', pointerEvents:'none' }}>SETTINGS</div>}
        <div style={{ position:'relative', zIndex:1, display:'flex', alignItems:'center', gap:'16px', flexWrap:'wrap' }}>
          <div style={{ width: isMobile ? 56 : 72, height: isMobile ? 56 : 72, borderRadius:'50%', background:'#2a3f6a', border:'3px solid #C9A84C', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
            <span style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize: isMobile ? '18px' : '24px', color:'#C9A84C', letterSpacing:'2px' }}>{initials}</span>
          </div>
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize: isMobile ? 'clamp(22px,6vw,36px)' : 'clamp(24px,4vw,40px)', color:'#fff', letterSpacing:'2px', lineHeight:1, marginBottom:'3px' }}>{firstName.toUpperCase()} {lastName.toUpperCase()}</div>
            <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:'12px', color:'rgba(255,255,255,0.45)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{email}</div>
          </div>
        </div>
      </div>

      {/* CONTENT */}
      <div style={{ maxWidth: isMobile ? '100%' : '800px', margin:'0 auto', padding: isMobile ? '16px 12px 80px' : '28px 40px 80px' }}>

        {/* ACCOUNT */}
        <Section title="Account">
          <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap:'12px' }}>
            <Field label="First Name"><input style={inp} value={firstName} onChange={e => setFirstName(e.target.value)} placeholder="Ryan" onFocus={e => e.target.style.borderColor='#C9A84C'} onBlur={e => e.target.style.borderColor=t.border} /></Field>
            <Field label="Last Name"><input style={inp} value={lastName} onChange={e => setLastName(e.target.value)} placeholder="Groene" onFocus={e => e.target.style.borderColor='#C9A84C'} onBlur={e => e.target.style.borderColor=t.border} /></Field>
          </div>
          <Field label="Email" note="Your login email. Contact support to change it."><input style={inpRO} value={email} readOnly /></Field>
          <Field label="Bio"><textarea value={bio} onChange={e => setBio(e.target.value)} placeholder="One sentence about your racing journey..." style={{ ...inp, minHeight:'72px', resize:'vertical', lineHeight:1.6 }} onFocus={e => e.target.style.borderColor='#C9A84C'} onBlur={e => e.target.style.borderColor=t.border} /></Field>
          <SaveButton section="account" updates={{ bio }} />
        </Section>

        {/* PASSPORT INFO */}
        <Section title="Race Passport Info">
          <p style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:'12px', color:t.textMuted, lineHeight:1.6, marginBottom:'14px', marginTop:0 }}>Used to pre-fill race registrations when direct registration launches.</p>
          <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap:'12px' }}>
            <Field label="Phone"><input style={inp} type="tel" value={phone} onChange={e => setPhone(e.target.value)} placeholder="+1 (555) 000-0000" onFocus={e => e.target.style.borderColor='#C9A84C'} onBlur={e => e.target.style.borderColor=t.border} /></Field>
            <Field label="Bib Name" note="Leave blank to use your full name."><input style={inp} value={raceName} onChange={e => setRaceName(e.target.value)} placeholder="Ryan" onFocus={e => e.target.style.borderColor='#C9A84C'} onBlur={e => e.target.style.borderColor=t.border} /></Field>
          </div>
          <Field label="Date of Birth"><DobPicker value={dob} onChange={setDob} t={t} /></Field>
          <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap:'12px' }}>
            <Field label="Gender">
              <select style={sel} value={gender} onChange={e => setGender(e.target.value)} onFocus={e => e.target.style.borderColor='#C9A84C'} onBlur={e => e.target.style.borderColor=t.border}>
              ...
              <div style={{ display:'flex', gap:'4px', flexWrap:'wrap' }}>
                {SIZES.map(s => (
                  <button key={s} onClick={() => setShirtSize(s===shirtSize?'':s)}
                    style={{ flex:1, minWidth:'36px', padding:'10px 4px', borderRadius:'6px', border:`1.5px solid ${shirtSize===s?'#1B2A4A':t.border}`, background:shirtSize===s?'#1B2A4A':t.inputBg, color:shirtSize===s?'#fff':t.textMuted, fontFamily:"'Barlow Condensed',sans-serif", fontSize:'11px', fontWeight:600, cursor:'pointer' }}>{s}</button>
                ))}
              </div>
            </Field>
          </div>
          <Field label="Street Address"><input style={inp} value={address} onChange={e => setAddress(e.target.value)} placeholder="123 Main St" onFocus={e => e.target.style.borderColor='#C9A84C'} onBlur={e => e.target.style.borderColor=t.border} /></Field>
          {/* Address grid — 2 col on mobile, 4 col on desktop */}
          <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : '1fr 1fr 1fr 1.5fr', gap:'10px' }}>
            <Field label="City"><input style={inp} value={city} onChange={e => setCity(e.target.value)} placeholder="Highland" onFocus={e => e.target.style.borderColor='#C9A84C'} onBlur={e => e.target.style.borderColor=t.border} /></Field>
            <Field label="State"><input style={inp} value={state} onChange={e => setState(e.target.value)} placeholder="MD" onFocus={e => e.target.style.borderColor='#C9A84C'} onBlur={e => e.target.style.borderColor=t.border} /></Field>
            <Field label="Zip"><input style={inp} value={zip} onChange={e => setZip(e.target.value)} placeholder="20777" onFocus={e => e.target.style.borderColor='#C9A84C'} onBlur={e => e.target.style.borderColor=t.border} /></Field>
            <Field label="Country"><input style={inp} value={country} onChange={e => setCountry(e.target.value)} placeholder="United States" onFocus={e => e.target.style.borderColor='#C9A84C'} onBlur={e => e.target.style.borderColor=t.border} /></Field>
          </div>
        ...
        <Section title="Emergency Contact">
          <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap:'12px' }}>
            <Field label="Contact Name"><input style={inp} value={contactName} onChange={e => setContactName(e.target.value)} placeholder="Full name" onFocus={e => e.target.style.borderColor='#C9A84C'} onBlur={e => e.target.style.borderColor=t.border} /></Field>
            <Field label="Contact Phone"><input style={inp} type="tel" value={contactPhone} onChange={e => setContactPhone(e.target.value)} placeholder="+1 (555) 000-0000" onFocus={e => e.target.style.borderColor='#C9A84C'} onBlur={e => e.target.style.borderColor=t.border} /></Field>
          </div>
        ...
        <Section title="Running Background">
          <Field label="Experience Level">
            <select style={sel} value={experience} onChange={e => setExperience(e.target.value)} onFocus={e => e.target.style.borderColor='#C9A84C'} onBlur={e => e.target.style.borderColor=t.border}>
            ...
            <div style={{ display:'flex', gap:'6px', flexWrap:'wrap' }}>
              {DISTANCES.map(d => (
                <button key={d} onClick={() => setFavoriteDistance(d===favoriteDistance?'':d)}
                  style={{ padding:'9px 8px', borderRadius:'6px', border:`1.5px solid ${favoriteDistance===d?'#1B2A4A':t.border}`, background:favoriteDistance===d?'#1B2A4A':t.inputBg, color:favoriteDistance===d?'#fff':t.textMuted, fontFamily:"'Barlow Condensed',sans-serif", fontSize:'11px', fontWeight:600, cursor:'pointer', flexGrow:1, textAlign:'center' }}>{d}</button>
              ))}
            </div>
          </Field>
          <Field label="Have you run a marathon?"><YesNo value={doneMarathon} onChange={setDoneMarathon} t={t} /></Field>
          {doneMarathon === true && <Field label="Have you completed an IRONMAN?"><YesNo value={doneIronman} onChange={setDoneIronman} t={t} /></Field>}
        ...
        <Section title="Race History">
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:'16px', padding:'14px', background:t.surfaceAlt, border:`1.5px solid ${t.border}`, borderRadius:'10px', flexWrap: isMobile ? 'wrap' : 'nowrap' }}>
            <div>
              ...
              <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:'12px', color:t.textMuted, lineHeight:1.5 }}>Run the import again to add missing races or pull in new results.</div>
            </div>
            <button onClick={() => navigate('/race-import')}
              style={{ padding:'9px 20px', border:'1.5px solid #1B2A4A', borderRadius:'8px', background:'transparent', fontFamily:"'Barlow Condensed',sans-serif", fontSize:'11px', fontWeight:600, letterSpacing:'1.5px', color:'#1B2A4A', cursor:'pointer', textTransform:'uppercase', whiteSpace:'nowrap', flexShrink:0 }}
        ...
        <Section title="Connected Accounts">
          <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
            {[
              ...
              <div key={acct.name} style={{ display:'flex', alignItems:'center', gap:'12px', padding:'12px 14px', background:t.surfaceAlt, border:`1.5px solid ${t.border}`, borderRadius:'10px' }}>
                <div style={{ width:36, height:36, borderRadius:'8px', background:t.isDark?'rgba(255,255,255,0.06)':'rgba(27,42,74,0.06)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>{acct.icon}</div>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:'13px', fontWeight:600, color:t.text }}>{acct.name}</div>
                  {!isMobile && <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:'11px', color:t.textMuted }}>{acct.desc}</div>}
                </div>
                <button style={{ padding:'6px 14px', border:`1.5px solid ${t.text}`, borderRadius:'7px', background:'transparent', fontFamily:"'Barlow Condensed',sans-serif", fontSize:'11px', fontWeight:600, letterSpacing:'1.5px', color:t.text, cursor:'pointer', textTransform:'uppercase', whiteSpace:'nowrap' }}
        ...
        <Section title="Danger Zone" danger>
          <div style={{ display:'flex', alignItems: isMobile ? 'flex-start' : 'center', justifyContent:'space-between', gap:'16px', flexDirection: isMobile ? 'column' : 'row' }}>
            ...
            <button style={{ padding:'8px 20px', border:'1.5px solid rgba(197,48,48,0.4)', borderRadius:'8px', background:'transparent', fontFamily:"'Barlow Condensed',sans-serif", fontSize:'11px', fontWeight:600, letterSpacing:'1.5px', color:'#c53030', cursor:'pointer', textTransform:'uppercase', whiteSpace:'nowrap', flexShrink:0 }}
